...
def main():
    ...
    parser.add_argument('--obs_length', type=int, default=180,
    ...
    for iterator in range(50):
        ...
        for batch in range(dataloader.num_batches):
            ...
            error = get_mean_error(x_seq[sample_args.obs_length:], ret_x_seq[sample_args.obs_length:], False)
            total_error += error
...
def sample(args, x_seq, net):
    ...
    with torch.no_grad():
        ...
        ret_x_seq = Variable(torch.zeros(args.obs_length+args.pred_length + extra_pred_length, net.input_size))
        ...
        #copy all obs data
        ret_x_seq[:args.obs_length, :] = x_seq.clone()[:args.obs_length, :]
        ret_x_seq[:,3:] = x_seq[:,3:]
        ...
        for tstep in range(args.obs_length-1, extra_pred_length + args.pred_length + args.obs_length-1):
            outputs, hidden_states, cell_states = net(ret_x_seq[tstep].view(1, 1, net.input_size), hidden_states, cell_states)
